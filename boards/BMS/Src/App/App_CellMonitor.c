...
void App_CellMonitor_Configure(const struct CellMonitor *cell_monitoring)
{
    cell_monitoring->configure_daisy_chain();
}

ExitCode App_CellMonitor_ReadCellVoltages(
    const struct CellMonitor *const cell_monitoring)
{
    return cell_monitoring->read_cell_voltages();
}
