...
static const uint16_t crc_lut[UINT8_MAX + 1] = {
...
uint16_t Io_CalculatePec15(uint8_t *data_buffer, uint32_t size)
{
    ...
    for (size_t i = 0U; i < size; i++)
    {
        ...
        pec15_remainder =
            (uint16_t)((pec15_remainder << 8) ^ crc_lut[pec15_lut_index]);
    }
...
{
    // The command used to write payload data to configuration register A.
    ...
    if (Io_SharedSpi_MultipleTransmitWithoutNssToggle(
            spi_interface, tx_payload, 8U, NUM_OF_LTC6813) != HAL_OK)
    {
        Io_SharedSpi_SetNssHigh(spi_interface);
        return EXIT_CODE_ERROR;
    }
...
{
    // The command used to start die temperature conversions.
    uint32_t ADSTAT = (0x460 + (MD << 7) + CHST);
