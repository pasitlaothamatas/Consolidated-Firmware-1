...
void Io_SteeringAngleSensor_AdcConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    {
        steering_angle_sensor.angle = (float)((_Io_SharedAdc_GetChannelVoltage(steering_angle_sensor.adc_input, steering_angle_sensor.rank) - steering_angle_sensor.offset_voltage) / DEGREES_PER_VOLT);
    }
}
